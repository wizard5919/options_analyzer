...
def calculate_dynamic_thresholds(stock_data: pd.Series, side: str, is_0dte: bool) -> Dict[str, float]:
    """Calculate dynamic thresholds with consistent 'delta_min' key for magnitude."""
    ...
    # Less aggressive multiplier to fix high thresholds in tests
    vol_multiplier = 1 + (volatility * 50)  # Changed from *100 to *50 for better balance
    
    # Calculate delta magnitude (positive value for both sides)
    magnitude = abs(thresholds['delta_base']) * vol_multiplier
    magnitude = max(0.3, min(0.8, magnitude))
    thresholds['delta_min'] = magnitude  # Magnitude for |delta|
...
def generate_enhanced_signal(option: pd.Series, side: str, stock_df: pd.DataFrame, is_0dte: bool) -> Dict:
    ...
    try:
        ...
        ema_9 = float(latest['EMA_9']) if not pd.isna(latest['EMA_9']) else close  # Fallback to close if NaN
        ema_20 = float(latest['EMA_20']) if not pd.isna(latest['EMA_20']) else close  # Fallback
        rsi = float(latest['RSI']) if not pd.isna(latest['RSI']) else 50.0  # Neutral fallback
        ...
        if side == "call":
            # Delta condition (using magnitude)
            ...
            explanations.append({
                ...
                'score': delta_score,
                'explanation': f"Delta {delta:.3f} {'✓' if delta_pass else '✗'} threshold {thresholds['delta_min']:.2f}."
            })
            ...
            conditions.append((trend_pass, "Price > EMA9 > EMA20", f"{close:.2f} > {ema_9:.2f} > {ema_20:.2f}"))
            explanations.append({
                'condition': 'Trend',
                'passed': trend_pass,
                'value': f"Price: {close:.2f}, EMA9: {ema_9:.2f}, EMA20: {ema_20:.2f}",
            ...
            explanations.append({
                ...
                'passed': delta_pass,
                'value': delta,
                'threshold': -thresholds['delta_min'],  # Show negative for put
                'weight': weights['delta'],
                'score': delta_score,
                'explanation': f"Delta {delta:.3f} {'✓' if delta_pass else '✗'} threshold {-thresholds['delta_min']:.2f}."
            })
...
with st.sidebar:
    ...
    with st.expander("⚡ Performance Tips"):
        ...
        **🚀 Speed Optimizations:**
        ...
        - Combined dependencies: Stock data + indicators computed together
