...
CONFIG = {
    ...
    'SR_SENSITIVITY': {
        '1min': 0.002,  # Increased from 0.001
        '5min': 0.004,  # Increased from 0.002
        '15min': 0.006, # Increased from 0.003
        '30min': 0.010, # Increased from 0.005
        '1h': 0.016     # Increased from 0.008
    },
    'SR_WINDOW_SIZES': {
        '1min': 3,
        '5min': 3,
...
def cluster_levels_improved(levels: List[float], current_price: float, sensitivity: float, level_type: str) -> List[Dict]:
    ...
    try:
        ...
        # Return only the top 2-3 strongest levels
        return clustered[:3]  # Changed from [:5] to [:3]
...
def is_market_open() -> bool:
    """Check if current Eastern time is within trading hours (9:30am–4:00pm ET, Mon–Fri)."""
...
def is_extended_hours() -> bool:
    """Check if we're in extended trading hours (4 AM to 8 PM ET)"""
    try:
        eastern = pytz.timezone('US/Eastern')
        now = datetime.datetime.now(eastern)
        now_time = now.time()
        ...
        return datetime.time(4, 0) <= now_time <= datetime.time(20, 0)
    except Exception:
        return False
...
with tab1: # General tab
    ...
    if ticker:
        ...
        if 'last_extended_hours_status' not in st.session_state:
            st.session_state.last_extended_hours_status = is_extended_hours()
        if 'frozen_cache_age' not in st.session_state:
            st.session_state.frozen_cache_age = 0
        # Determine current market status
        current_extended_hours = is_extended_hours()
        # Update cache age based on market status
        if current_extended_hours:
            # Market is open - update cache age normally
            cache_age = int(time.time() - st.session_state.last_refresh)
        else:
            # Market is closed - use frozen cache age
            cache_age = st.session_state.frozen_cache_age
        # If we just transitioned from extended to non-extended hours, freeze the cache age
        if st.session_state.last_extended_hours_status and not current_extended_hours:
...
with tab5: # Technical tab
    ...
    if ticker:
        ...
        if st.session_state.sr_data:
            ...
            # Detailed levels - show only the strongest 2-3 per timeframe
