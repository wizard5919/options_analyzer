...
def calculate_support_resistance_enhanced(data: pd.DataFrame, timeframe: str, current_price: float) -> dict:
    ...
    try:
        ...
        resistance_levels = [float(highs[i]) for i in all_resistance_indices if i < len(highs) and not math.isnan(highs[i])]
        support_levels = [float(lows[i]) for i in all_support_indices if i < len(lows) and not math.isnan(lows[i])]
        ...
        resistance_levels.extend([float(closes[i]) for i in close_peaks if i < len(closes) and not math.isnan(closes[i])])
        support_levels.extend([float(closes[i]) for i in close_valleys if i < len(closes) and not math.isnan(closes[i])])
...
def validate_sr_alignment(results: dict, current_price: float):
    ...
    try:
        ...
        for tf, data in results.items():
            ...
            valid_support = [level for level in support_levels 
                            if not math.isnan(level) and level < current_price]
            valid_resistance = [level for level in resistance_levels 
                              if not math.isnan(level) and level > current_price]
...
# UPDATED: Enhanced price fetching with async parallel calls
@st.cache_data(ttl=5, show_spinner=False)
def get_current_price(ticker: str) -> float:
    """Get real-time price from multiple free sources in parallel using async"""
    # Try Polygon first if available (sync for now)
    ...
    # Define async fetch functions for free APIs
    async def fetch_av(session):
        if can_make_request("ALPHA_VANTAGE") and CONFIG['ALPHA_VANTAGE_API_KEY']:
    ...
    async def fetch_fmp(session):
        if can_make_request("FMP") and CONFIG['FMP_API_KEY']:
    ...
    async def fetch_iex(session):
        if can_make_request("IEX") and CONFIG['IEX_API_KEY']:
    ...
    async def fetch_parallel():
        async with aiohttp.ClientSession() as session:
            tasks = [fetch_av(session), fetch_fmp(session), fetch_iex(session)]
    ...
    parallel_price = asyncio.run(fetch_parallel())
    if parallel_price is not None:
        return parallel_price

    # Yahoo Finance fallback (sync)
    try:
...
def generate_enhanced_signal(option: pd.Series, side: str, stock_df: pd.DataFrame, is_0dte: bool) -> Dict:
    ...
    try:
        ...
        if side == "call":
            ...
            conditions.append((treind_pass, "Price > EMA9 > EMA20", f"{close:.2f} > {ema_9:.2f} > {ema_20:.2f}" if ema_9 and ema_20 else "N/A"))
            explanations.append({
                'condition': 'Trend',
                'passed': trend_pass,
...
def get_sentiment_score(ticker: str) -> float:
    """Get sentiment score for a ticker using X scraping with Playwright"""
    positive_words = ['good', 'excellent', 'positive', 'buy', 'up', 'bullish', 'strong', 'gain', 'rise']
    negative_words = ['bad', 'poor', 'negative', 'sell', 'down', 'bearish', 'weak', 'loss', 'fall']
    ...
    try:
        with sync_playwright() as pw:
            ...
            search_url = f"https://x.com/search?q={ticker}%20stock&src=typed_query&f=live"
            page.goto(search_url)
            page.wait_for_selector("[data-testid='tweet']", timeout=10000)
            tweets = page.query_selector_all("[data-testid='tweetText']")
            texts = [t.inner_text() for t in tweets[:10] if t]  # Get up to 10 tweets
            browser.close()
        ...
        return float(np.tanh(avg_score * 5))  # Scale to -1 to 1

    except Exception as e:
        st.warning(f"Sentiment scraping error: {str(e)}. Falling back to price-based simulation.")
        ...
        return float(np.tanh(five_day_return * 5))
...
def process_options_batch(options_df: pd.DataFrame, side: str, stock_df: pd.DataFrame, current_price: float) -> pd.DataFrame:
    ...
    try:
        ...
        # Add Greek columns if missing
        for col in ['delta', 'gamma', 'theta', 'vega']:
            if col not in options_df.columns:
                options_df[col] = np.nan
        ...
        for idx, row in options_df.iterrows():
            if pd.isna(row['delta']) or pd.isna(row['gamma']) or pd.isna(row['theta']):
                delta, gamma, theta = calculate_approximate_greeks(
                    row.to_dict(), current_price
                )
...
# NEW: User authentication for production
# Define credentials (in production, use st.secrets or database)
credentials = {
    "usernames": {
        "user1": {
            "name": "User One",
            "password": "$2b$12$EixZaYVK1fsbw1ZfbQ6ZeGeaFJPuTVhRCgnES7O4qinfwBdK6f4TO"  # hashed '12345'
        }
    }
}
authenticator = stauth.Authenticate(credentials, "app_cookie", "abcdef", cookie_expiry_days=30)
...
with st.sidebar:
    ...
    # NEW: Free API API Keys
    st.subheader("🔑 Free API Keys")
    st.info("Use these free APIs to reduce rate limits")
...
# NEW: Unit Tests Section
class TestSignalGeneration(unittest.TestCase):
    def test_call_signal(self):
        # Mock data for a positive call signal
        mock_option = pd.Series({
            'delta': 0.6,
            'gamma': 0.06,
            'theta': 0.04,
            'volume': 1500,
        ...
        signal = generate_enhanced_signal(mock_option, 'call', mock_stock_df, False)
        self.assertTrue(signal['signal'])
        self.assertGreater(signal['score'], 0.5)
        self.assertIsNotNone(signal['profit_target'])
        self.assertIsNotNone(signal['stop_loss'])

    def test_put_signal_failure(self):
        # Mock data for a failing put signal
        mock_option = pd.Series({
            'delta': -0.4,
            'gamma': 0.04,
            'theta': 0.06,
        ...
        signal = generate_enhanced_signal(mock_option, 'put', mock_stock_df, False)
        self.assertFalse(signal['signal'])
        self.assertLess(signal['score'], 0.5)

# Run tests if script is run directly (for development)
