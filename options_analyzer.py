...
def calculate_profit_potential(option_data: pd.Series, current_price: float, side: str, price_moves: List[float]) -> Dict:
    ...
    try:
        ...
        for move in price_moves:
            ...
            if side == 'call':
                ...
                profit = (new_option_price - premium) * 100
...
def get_top_profit_opportunities(calls_df: pd.DataFrame, puts_df: pd.DataFrame, 
                               current_price: float, price_moves: List[float], top_n: int = 5) -> Dict:
    """Identify top profit opportunities"""
    opportunities = []
    
    # Analyze calls
    for _, call in calls_df.iterrows():
        profit_analysis = calculate_profit_potential(call, current_price, 'call', price_moves)
    ...
    for _, put in puts_df.iterrows():
        profit_analysis = calculate_profit_potential(put, current_price, 'put', price_moves)
        if 'error' not in profit_analysis:
    ...
    with tab2:
        try:
            with st.spinner("Generating trading signals..."):
                ...
                st.write("**Call Signals**")
                call_signals = []
                for _, option in calls_filtered.iterrows():
                    signal = generate_signal(option, 'call', df)
                    if signal['signal']:
                        call_signals.append({
                            'Contract': option['contractSymbol'],
                            'Strike': f"${option['strike']:.2f}",
                            'Premium': f"${option['lastPrice']:.2f}",
                            'Moneyness': classify_moneyness(option['strike'], current_price),
                            'Signal Score': f"{signal['score']:.2%}",
                            'Passed': ", ".join(signal['passed_conditions']),
                            'Failed': ", ".join(signal['failed_conditions']) if signal['failed_conditions'] else "None"
                        })
                if call_signals:
                    st.dataframe(pd.DataFrame(call_signals), use_container_width=True)
                else:
                    st.info("No call signals meet the criteria.")
                st.write("**Put Signals**")
                put_signals = []
                for _, option in puts_filtered.iterrows():
                    signal = generate_signal(option, 'put', df)
                    if signal['signal']:
                        put_signals.append({
                            'Contract': option['contractSymbol'],
                            'Strike': f"${option['strike']:.2f}",
                ...
                if put_signals:
                    st.dataframe(pd.DataFrame(put_signals), use_container_width=True)
                else:
                    st.info("No put signals meet the criteria.")
        except Exception as e:
            st.error(f"Error generating signals: {str(e)}")
    
    with tab3:
        try:
            with st.spinner("Generating real-time chart..."):
                df = get_stock_data(ticker)
                if df.empty:
                    st.error("Unable to fetch stock data.")
                    st.stop()
                df = compute_indicators(df)
                fig = create_real_time_chart(df, ticker)
        ...
        try:
            with st.spinner("Fetching detailed data..."):
                ...
                if not expiries:
                    st.error("No options expiries available.")
                    st.stop()
                st.subheader("⚙️ Detailed Options Data")
                selected_expiry = st.selectbox("Select Expiry Date", expiries)
                calls, puts = fetch_options_data(ticker, [selected_expiry])
                if calls.empty and puts.empty:
                    st.error("No options data available for selected expiry.")
                else:
                    st.write("**Calls**")
                    if not calls.empty:
                        calls_display = calls[['contractSymbol', 'strike', 'lastPrice', 'delta', 'gamma', 'theta', 'volume', 'openInterest', 'expiry']].copy()
                    ...
                    st.write("**Puts**")
                    if not puts.empty:
                        puts_display = puts[['contractSymbol', 'strike', 'lastPrice', 'delta', 'gamma', 'theta', 'volume', 'openInterest', 'expiry']].copy()
                        puts_display['moneyness'] = puts_display['strike'].apply(lambda x: classify_moneyness(x, current_price))
                        puts_display = puts_display.rename(columns={
                            'contractSymbol': 'Contract',
                            'strike': 'Strike',
                            'lastPrice': 'Premium',
                            'delta': 'Delta',
