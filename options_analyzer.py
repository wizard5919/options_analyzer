...
try:
    ...
    from ta.volatility import AverageTrueRange, KeltnerChannel  # type: ignore
    TA_AVAILABLE = True
except Exception:
    RSIIndicator = EMAIndicator = MACD = AverageTrueRange = KeltnerChannel = None  # type: ignore
    TA_AVAILABLE = False
...
CONFIG = {
    ...
    'MIN_OPTION_PRICE': 0.25,  # CHANGED FROM 1.00 TO 0.25 FOR QUICK SCALPING
    'MIN_OPEN_INTEREST': 1000,
    'MIN_VOLUME': 100,
    'MAX_BID_ASK_SPREAD_PCT': 0.10,
}
...
SIGNAL_THRESHOLDS = {
    'call': {
        ...
        'volume_min': 500,  # Reduced for scalping
        'condition_weights': {
            'delta': 0.25,
            'gamma': 0.20,
            'theta': 0.15,
...
# NEW: Profit Target Alert Function
def check_profit_target_and_alert(entry_price: float, current_price: float, option_type: str) -> Tuple[bool, str]:
    """
    Check if profit target is hit and return alert message
    Returns: (target_hit, alert_message)
    """
    try:
...
def compute_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        df = df.copy()
        # If the optional `ta` library isn't available, fall back to pure-pandas indicator math.
        if not TA_AVAILABLE:
            close = df['Close']
            high  = df['High']
            low   = df['Low']
            vol   = df.get('Volume', pd.Series(index=df.index, dtype='float64')).fillna(0)
            ...
            # Keltner Channels
            kc_len = int(CONFIG.get('KC_LENGTH', 20))
            kc_mult = float(CONFIG.get('KC_MULTIPLIER', 1.5))
            typical = (high + low + close) / 3.0
            basis = typical.ewm(span=kc_len, adjust=False).mean()
            df['KC_middle'] = basis
            df['KC_upper']  = basis + kc_mult * df['ATR']
            df['KC_lower']  = basis - kc_mult * df['ATR']
            ...
            cum_vol = vol.cumsum()
            df['VWAP'] = (tp.mul(vol).cumsum() / cum_vol.replace(0, np.nan)).ffill()
...
def get_fallback_options_data(ticker: str) -> Tuple[List[str], pd.DataFrame, pd.DataFrame]:
    ...
    strike_range = max(2, current_price * 0.02) # 2% range or minimum $2 for scalping
    strikes = []
  
    # Generate strikes in reasonable increments
    if current_price < 50:
        increment = 0.5  # Tighter increments for scalping
    ...
    for strike in np.arange(start_strike, end_strike + increment, increment):
        if strike > 0:
    ...
    for expiry in expiries:
        ...
        for strike in strikes:
            ...
            call_price = max(0.25, intrinsic_call + time_value * gamma)  # Minimum $0.25
            put_price = max(0.25, intrinsic_put + time_value * gamma)    # Minimum $0.25
          
            # Volume estimates
            volume = 500 if abs(moneyness - 1) < 0.02 else 250 # Higher volume near ATM
            ...
            calls_data.append({
                ...
                'impliedVolatility': 0.30 if is_0dte else 0.25,
...
def classify_moneyness(strike: float, spot: float) -> str:
    ...
    try:
        ...
        if diff_pct < 0.005: # Within 0.5%
            ...
            if diff_pct < 0.02: # 0.5-2% below
                return 'NTM' # Near-the-money
            else:
                return 'ITM'
        else: # Above current price
            if diff_pct < 0.02: # 0.5-2% above
                return 'NTM' # Near-the-money
            else:
                return 'OTM'
    except Exception:
...
# UPDATED: Enhanced validation with liquidity filters for scalping
def validate_option_data(option: pd.Series, spot_price: float) -> bool:
    ...
    try:
        ...
        # CHANGED: Lower minimum price for quick scalping (from 1.00 to 0.25)
        if option['lastPrice'] < CONFIG['MIN_OPTION_PRICE']:
            return False
        
        # Check for realistic bid/ask
        ...
        # Allow slightly wider spreads for cheaper options (scalping-friendly)
        max_spread = CONFIG['LIQUIDITY_THRESHOLDS']['max_bid_ask_spread_pct']
        if option['lastPrice'] < 0.5:
            max_spread *= 2.0  # Allow 2x wider spread for cheap options for scalping
...
def process_options_batch(options_df: pd.DataFrame, side: str, stock_df: pd.DataFrame, current_price: float) -> pd.DataFrame:
    ...
    try:
        ...
        options_df = options_df[options_df['lastPrice'] >= CONFIG['MIN_OPTION_PRICE']]  # Filter by minimum price
        ...
        if signals:
            ...
            # Sort by 0DTE first, then by score for scalping focus
...
with st.sidebar:
    ...
    with st.expander("📊 Signal Thresholds & Weights", expanded=False):
        ...
        with col1:
            ...
            SIGNAL_THRESHOLDS['call']['volume_min'] = st.slider("Call Min Volume", 100, 5000, 500, 100, key="call_vol_min")
        ...
        with col2:
            ...
            SIGNAL_THRESHOLDS['put']['volume_min'] = st.slider("Put Min Volume", 100, 5000, 500, 100, key="put_vol_min")
    ...
    with st.expander("💰 Liquidity Filters", expanded=False):
        CONFIG['MIN_OPTION_PRICE'] = st.slider(
            ...
            0.05, 5.0, 0.25, 0.05,  # CHANGED: Default to 0.25 for scalping
    ...
    with tab1:
        try:
            with st.spinner("🔄 Loading enhanced analysis..."):
                ...
                # Expiry selection - CHANGED DEFAULT TO 0DTE ONLY
                col1, col2 = st.columns(2)
                with col1:
                    expiry_mode = st.radio(
                        "📅 Expiration Filter:",
                        ["0DTE Only", "This Week", "All Near-Term"],
                        index=0,  # CHANGED: Default to 0DTE Only for scalping
                ...
                # Strike range filter - CHANGED DEFAULT TO TIGHTER RANGE
                strike_range = st.slider(
                    "🎯 Strike Range Around Current Price ($):",
                    -10.0, 10.0, (-2.0, 2.0), 0.5,  # CHANGED: Tighter default for scalping
                ...
                # Moneyness filter - CHANGED DEFAULT TO FOCUS ON ATM/NTM
                m_filter = st.multiselect(
                    "💰 Moneyness Filter:",
                    options=["ITM", "NTM", "ATM", "OTM"],
                    default=["ATM", "NTM"],  # CHANGED: Focus on ATM and near-the-money for scalping
                        ...
                        if not call_signals_df.empty:
                            ...
                            for idx, row in call_signals_df.iterrows():
                                target_hit, alert_msg = check_profit_target_and_alert(
                                    row['lastPrice'], 
                                    row.get('current_price', row['lastPrice']),  # Use current_price if available, else lastPrice
                                    'call'
                                )
                                if target_hit:
