...
def calculate_scanner_score(stock_df: pd.DataFrame, side: str) -> float:
    ...
    try:
        ...
        return 0.0
def create_stock_chart(df: pd.DataFrame, sr_levels: dict = None, timeframe: str = '5m') -> go.Figure:
...
if ticker:
    ...
    tab_general, tab_chart, tab_news, tab_financials, tab_technical, tab_forum = st.tabs([
        "General",
        "Chart",
        "News & Analysis",
        "Financials",
    ...
    with tab_chart:
        ...
        tf_config = {
            ...
            '4H': {'interval': '60m', 'period': '1y', 'resample': '4H'},
            '1D': {'interval': '1d', 'period': '5y'},
            '1W': {'interval': '1wk', 'period': '10y'},
            '1M': {'interval': '1mo', 'period': 'max'}
        }
      
        params = tf_config.get(selected_tf, tf_config['5m'])
        ...
        if not chart_df.empty:
            ...
            if chart_fig:
                ...
                st.warning("⚠️ Unable to create chart. Data may be insufficient.")
        else:
            st.error("❌ Unable to fetch chart data for selected timeframe.")
    ...
    with tab_financials:
        ...
        try:
            ...
            st.warning(f"⚠️ Financial data unavailable: {str(e)}")
...
if st.session_state.get('auto_refresh_enabled', False) and ticker:
    ...
    with st.expander("📚 Quick Start Guide", expanded=False):
        ...
        **🔧 Optimization:**
        ...
        - **Conservative Refresh**: Use 120s+ intervals to prevent limits
